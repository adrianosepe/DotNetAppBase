...
namespace Grapp.AppBase.Std.Library
{
	public partial class XHelper
	{
		...
		{
			public static LinqExpressions.Expression<Func<TModel, bool>> BuildExpression<TModel>(LinqExpressions.Expression<Func<TModel, bool>> value)
			{
				return value;
			}
...
// ReSharper restore AssignNullToNotNullAttribute
			...
			public static LinqExpressions.ExpressionType GetExpressionType(LinqExpressions.LambdaExpression memberSelector)
			{
				return memberSelector.Body.NodeType;
			}
			...
			public static string GetMemberName<T>(LinqExpressions.Expression<Func<T, object>> expression)
			{
				return GetMemberName(expression.CastTo<LinqExpressions.LambdaExpression>());
			}
            ...
            public static TItem GetMemberValueOfType<T, TItem>(LinqExpressions.Expression<Func<T, TItem>> memberSelector, T obj)
			{
				return memberSelector.Compile()(obj);
			}
			...
			{
				return GetExpressionType(expression) == LinqExpressions.ExpressionType.Convert &&
			...
			public static bool IsEquals(string exp1AsString, LinqExpressions.LambdaExpression exp2)
			{
				return exp1AsString == GetMemberName(exp2);
			}
