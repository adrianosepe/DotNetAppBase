...
// ReSharper restore CheckNamespace
{
	...
	{
		return row.GetBool(field, defaultValue: false);
	}

	public static T? GetNullable<T>(this DataRow dataRow, string fieldName) where T : struct
	{
		return GetNullable<T>(dataRow, fieldName, defaultValue: null);
	}
	...
	public static T? GetNullable<T>(this DataRow dataRow, string fieldName, T? defaultValue) where T : struct
	{
		...
		return value is DBNull ? defaultValue : (T)Convert.ChangeType(value, conversionType: typeof(T));
	}
	...
	public static bool TryGet<T>(this DataRow dataRow, string fieldName, out T outValue)
	{
		...
		outValue = (T)Convert.ChangeType(value, conversionType: typeof(T));
		return true;
	}
