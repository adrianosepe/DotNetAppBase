...
// ReSharper restore CheckNamespace
{
	...
	{
		return new DateTime(date.Year, date.Month, date.Day, hour: 0, minute: 0, second: 0, millisecond: 0, date.Kind);
	}
	...
	{
		return new DateTime(date.Year, date.Month, date.Day, hour: 23, minute: 59, second: 59, millisecond: 999, date.Kind);
	}
	...
	{
		return current.SetDate(current.Year, 1, day: 1);
	}
	...
	{
		var nextMonth = date.AddMonths(months: 1);
		return new DateTime(nextMonth.Year, nextMonth.Month, day: 1).AddDays(value: -1).Day;
	}
	...
	{
		return new DateTime(date.Year, date.Month, day: 1);
	}
	...
	{
		return new DateTime(date.Year, date.Month, day: GetCountDaysOfMonth(date));
	}
	...
	{
		return current.FirstDayOfWeek().AddDays(value: 6);
	}
	...
	public static DateTime Round(this DateTime dateTime, ERoundTo rt)
	{
		...
		switch(rt)
		{
			case ERoundTo.Second:
			{
				...
				{
					rounded = rounded.AddSeconds(value: 1);
				}
				break;
			}
			case ERoundTo.Minute:
			{
				rounded = new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, dateTime.Hour, dateTime.Minute, second: 0, dateTime.Kind);
				if(dateTime.Second >= 30)
				{
					rounded = rounded.AddMinutes(value: 1);
				}
				break;
			}
			case ERoundTo.Hour:
			{
				rounded = new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, dateTime.Hour, minute: 0, second: 0, dateTime.Kind);
				if(dateTime.Minute >= 30)
				{
					rounded = rounded.AddHours(value: 1);
				}
				break;
			}
			case ERoundTo.Day:
			{
				rounded = new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, hour: 0, minute: 0, second: 0, dateTime.Kind);
				if(dateTime.Hour >= 12)
				{
					rounded = rounded.AddDays(value: 1);
				}
				break;
			}
			default:
			{
				throw new ArgumentOutOfRangeException(paramName: "rt");
			}
		}
	...
	/// <returns></returns>
	[Obsolete(message: "This method has been renamed to FirstDayOfWeek to be more consistent with existing conventions.")]
